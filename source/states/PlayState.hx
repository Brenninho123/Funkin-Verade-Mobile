...
{
	public static var STRUM_X:Float = 50;
	public static var STRUM_Y:Float = 50;
	...
	override public function create()
	{
		...
		STRUM_Y_DOWNSCROLL = (FlxG.height - 150); // Only works here
		...
		if (!Paths.fileExists('stages/$curStage.lua', TEXT) && !Paths.fileExists('stages/$curStage.hx', TEXT))
		{
			switch (curStage)
			{
				default: new StageWeek1();
			}
		}
		...
		if (eventNotes.length > 0)
		{
			...
			var preEvents:Array<EventNote> = [for (e in eventNotes) if ((e.strumTime - ClientPrefs.data.noteOffset) <= 0) e];
			for (event in preEvents)
			{
				triggerEvent(event.event, event.value1 ?? "", event.value2 ?? "", 0);
				eventNotes.shift();
			}
	...
	public inline function reloadHealthBarColors()
	{
		healthBar.setColors(
	...
	public var countdownPrepare:FlxSprite; // By default, the countdown in this mod has all 4 sprites
	...
	function cacheCountdown()
	{
		final introImagesArray:Array<String> = switch (stageUI)
		{
			...
			case "normal": ["count1", "count2", "count3", "count4"];
			default: ['${uiPrefix}UI/ready${uiPostfix}', '${uiPrefix}UI/set${uiPostfix}', '${uiPrefix}UI/go${uiPostfix}'];
		};
		for (asset in introImagesArray) Paths.image(asset);

		for (asset in ['intro3', 'intro2', 'intro1', 'introGo']) Paths.sound(asset + introSoundsSuffix);
	}
	...
	public function startCountdown()
	{
		...
		skipArrowStartTween = skipCountdown || startOnTime > 0;
		canPause = true;

		generateStaticArrows(0);
		generateStaticArrows(1);
		for (i=>s in playerStrums.members)
		{
			setOnScripts('defaultPlayerStrumX$i', s.x);
			setOnScripts('defaultPlayerStrumY$i', s.y);
		}
		for (i=>s in opponentStrums.members)
		{
			setOnScripts('defaultOpponentStrumX$i', s.x);
			setOnScripts('defaultOpponentStrumY$i', s.y);
		}
		...
		{
			final swagCounter:Int = (tmr.elapsedLoops - 1);
			...
			final intendedLen:Int = Math.round(Math.max((tmr.loops - 1), 0));

			final imgIndex:Int = (introImagesArray.length == intendedLen ? swagCounter : swagCounter - (intendedLen - introImagesArray.length));
			final countImage:String = introImagesArray[imgIndex] ?? "NONE";

			switch (swagCounter)
			{
				case 0:
					FlxG.sound.play(Paths.sound('intro3' + introSoundsSuffix), 0.6);
					if (countImage != "NONE") countdownPrepare = createCountdownSprite(countImage, antialias);
					tick = THREE;
				case 1:
					FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix), 0.6);
					if (countImage != "NONE") countdownReady = createCountdownSprite(countImage, antialias);
					tick = TWO;
				case 2:
					FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix), 0.6);
					if (countImage != "NONE") countdownSet = createCountdownSprite(countImage, antialias);
					tick = ONE;
				case 3:
	...
	private inline function createCountdownSprite(image:String, antialias:Bool):FlxSprite
	{
		final scale:Float = switch (isPixelStage)
		{
			case false: 0.8;
			case true: daPixelZoom;
		};

		var spr:FlxSprite = new FlxSprite(0, 0, Paths.image(image));
		spr.scale.set(scale, scale); spr.updateHitbox();
		spr.camera = camHUD;
		spr.screenCenter();
	...
	public function doScoreBop()
	{
		...
		if (scoreTxtTween?.active) FlxTween.completeTweensOf(scoreTxt, ["scale.x", "scale.y"]); // tween.finish() should exist imo

		var prevScale:FlxPoint = scoreTxt.scale.clone();
		scoreTxt.scale.add(0.075, 0.075);

		scoreTxtTween = FlxTween.tween(scoreTxt, {"scale.x": prevScale.x, "scale.y": prevScale.y}, 0.2, {ease: FlxEase.sineOut});
		prevScale.put();
	}
	...
	{
		if (!freezeCamera && !paused && !inCutscene) FlxG.camera.followLerp = (0.04 * cameraSpeed) * playbackRate;
		...
		if (camZooming)
		{
			final zoomingDecay:Float = Math.exp( ((-elapsed * 3.125) * camZoomingDecay) * playbackRate );
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, zoomingDecay);
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, zoomingDecay);
		}
		...
		if (controls.RESET && (!ClientPrefs.data.noReset && canReset) && !inCutscene && startedCountdown && !endingSong)
		{
			health = 0;
			trace("RESET = True");
		}
		...
		for (dunceNote in unspawnNotes)
		{
			...
			if (dunceNote.multSpeed < 1) time /= dunceNote.multSpeed;

			if ((dunceNote.strumTime - Conductor.songPosition) >= time) continue;
			...
			if (!inCutscene)
			{
				...
				if (startedCountdown)
				{
					...
					for (daNote in notes.members)
					{
						...
						{
							if (!daNote.mustPress) opponentNoteHit(daNote);
							else if (cpuControlled) goodNoteHit(daNote);
						}
						...
						{
							if ((daNote.mustPress && !cpuControlled) && !daNote.ignoreNote && (daNote.tooLate || !daNote.wasGoodHit) && !endingSong) noteMiss(daNote);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (isDead || (!skipHealthCheck && health > 0) || (practiceMode || gameOverTimer != null)) return false;
		...
		// Conquistas: Aprendizagem + Capa!
		Achievements.addScore('learner');
		Achievements.addScore('persistence');
		#end
		...
		{
			gameOverTimer = new FlxTimer().start(GameOverSubstate.deathDelay, function(tmr)
			{
				stopTheMusic();
		...
		switch(eventName) {
			...
			case 'Set Property':
				try
				{
					...
					if(split.length > 1) {
						LuaUtils.setVarInArray(LuaUtils.getPropertyLoop(split, true), split[split.length-1], trueValue);
					} else {
			...
			case 'Camera Fade':
				flValue2 ??= 1;
				var values:Array<String> = [for (v in value1.split(",")) v.trim()];

				#if LUA_ALLOWED
				final camName:String = values[0].toLowerCase();
				final target:FlxCamera = variables.exists(camName) ? variables[camName] : LuaUtils.cameraFromString(camName);
				#else
				final target:FlxCamera = switch (values[0].toLowerCase())
				{
	...
	public inline function moveCameraToGirlfriend()
	{
	...
	// Centering stuff whilst avoiding hardcoding, why does this need so much graah
	@:unreflective @:noCompletion private var ratingHeights:Array<Float> = [];
	@:unreflective @:noCompletion private var comboWidths:Array<Float> = [];
	...
	@:unreflective
	private static inline function averageCalc(values:Array<Float>):Float
	{
		var add:Float = 0;
	...
	private function popUpScore(note:Note):Rating
	{
		...
		final placementY:Float = switch (ClientPrefs.data.middleScroll)
		{
			case false: ClientPrefs.data.downScroll ? STRUM_Y_DOWNSCROLL : STRUM_Y;
			case true: 	ClientPrefs.data.downScroll ? STRUM_Y : STRUM_Y_DOWNSCROLL;
		};
		...
		for (i=>n in comboStr.split(""))
		{
			final numScale:Float = scale - 0.05;

			final scaleForCalc:Float = !isPixelStage ? numScale : numScale / (daPixelZoom / 2);
			final widthAvg:Float = (averageCalc(comboWidths) + 10) * scaleForCalc;
		...
		{
			var prevScale:FlxPoint = spr.scale.clone();
			spr.scale.add(0.045, 0.045);
			FlxTween.tween(spr, {"scale.x": prevScale.x, "scale.y": prevScale.y}, 0.2 / playbackRate, {ease: FlxEase.sineOut});

			FlxTween.tween(spr, {alpha: 0}, 1.2 / playbackRate,
			{
	...
	private function keyPressed(key:Int)
	{
		...
		var plrInputNotes:Array<Note> = notes.members.filter(function(n:Note) {
			final canHit:Bool = n != null && (!strumsBlocked[n.noteData] && !n.blockHit) && (n.mustPress && !n.wasGoodHit) && (n.canBeHit && !n.tooLate);
	...
	private function keysCheck()
	{
		...
		if (startedCountdown && generatedMusic && !inCutscene)
		{
			for (n in notes) // I can't do a filter here, that's kinda awesome
			{
				var canHit:Bool = n != null && (!strumsBlocked[n.noteData] && !n.blockHit) && (n.mustPress && !n.wasGoodHit) && (n.canBeHit && !n.tooLate);
				if (guitarHeroSustains) canHit = canHit && n.parent?.wasGoodHit;
				if ((!canHit || !n.isSustainNote) || !holdArray[n.noteData]) continue;
	...
	function opponentNoteHit(note:Note)
	{
		...
		final leData:Int = note.noteData;
		final leType:String = note.noteType;
		final leId:Int = notes.members.indexOf(note);

		var result:Dynamic = callOnLuas('opponentNoteHitPre', [leId, leData, leType, isSus]);
		if (result == LuaUtils.Function_StopAll || result == LuaUtils.Function_Stop) return;

		if (result != LuaUtils.Function_StopHScript) result = callOnHScript('opponentNoteHitPre', [note]);
		...
		else if(!note.noAnimation)
		{
			...
			var animToPlay:String = singAnimations[leData % singAnimations.length] + note.animSuffix;

			if (char != null)
			{
				var playHoldAnim:Bool = false;
				if (isSus)
				{
					final holdAnim:String = '$animToPlay-hold';
					if (playHoldAnim = char.hasAnimation(holdAnim)) animToPlay = holdAnim;
				}

				if (leType == 'Hey!' && char.hasAnimation('hey'))
				{
					char.playAnim('hey', true);
					char.specialAnim = true;
					char.heyTimer = 0.6;
				}
				else if (!char.getAnimationName().endsWith('-hold') && char.getAnimationName() != '$animToPlay-loop')
				{
					if (note.parent?.tail[note.parent?.tail.length - 1] == note)
						char.animPaused = false;
					else if (!isSus || playHoldAnim)
					{
						char.playAnim(animToPlay, true);
						if (note.tail.length > 0) char.animPaused = true;
					}
					char.holdTimer = 0;
				}
			}
		...
		strumPlayAnim(true, leData, ((Conductor.stepCrochet * 1.25) / 1000) / playbackRate);

		stagesFunc(function(stage:BaseStage) stage.opponentNoteHit(note));
		var result:Dynamic = callOnLuas('opponentNoteHit', [leId, leData, leType, isSus]);
	...
	public function goodNoteHit(note:Note)
	{
		...
		var result:Dynamic = callOnLuas('goodNoteHitPre', [leId, leData, leType, isSus]);
		if (result == LuaUtils.Function_StopAll || result == LuaUtils.Function_Stop) return;

		if (result != LuaUtils.Function_StopHScript) result = callOnHScript('goodNoteHitPre', [note]);
		if (result == LuaUtils.Function_Stop) return;
		...
		if (!note.hitCausesMiss) //Common notes
		{
			if (!note.noAnimation)
			{
				final char:Character = note.gfNote ? gf : boyfriend;
				var animToPlay:String = singAnimations[leData % singAnimations.length] + note.animSuffix;

				if (char != null)
				{
					var playHoldAnim:Bool = !char.animPaused;
					if (isSus)
					{
						final holdAnim:String = '$animToPlay-hold';
						if (playHoldAnim = char.hasAnimation(holdAnim)) animToPlay = holdAnim;
			...
			if (!cpuControlled) playerStrums.members[leData]?.playAnim('confirm', true);
			else strumPlayAnim(false, leData, (Conductor.stepCrochet * 1.25 / 1000) / playbackRate);
			vocals.volume = 1;

			final gainHealth:Bool = !guitarHeroSustains || !note.isSustainNote; // prevent health gain, *if* sustains are treated as a singular note
			if (gainHealth) health += note.hitHealth * healthGain;

			if ((!isSus && !note.ratingDisabled) && !cpuControlled)
			{
				++combo;
				songHits++;
				++totalPlayed;
		...
		{
			if (!note.noMissAnimation && leType == 'Hurt Note')
			{
	...
	override function beatHit()
	{
		...
		if ((camZooming && ClientPrefs.data.camZooms) && curBeat % camZoomingInterval == 0)
		{
			if (FlxG.camera.zoom < 1.35) FlxG.camera.zoom += 0.015 * camZoomingMult;
