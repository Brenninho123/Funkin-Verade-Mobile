...
class Native
{
	...
	public static function fixScaling():Void
	{
		...
		if (display != null)
		{
			...
			@:privateAccess Application.current.window.width = Std.int(FlxG.stage.stageWidth * dpiScale);
			@:privateAccess Application.current.window.height = Std.int(FlxG.stage.stageHeight * dpiScale);
