...
class MusicPlayer extends FlxGroup 
{
	...
	override function update(elapsed:Float)
	{
		...
		if (playing && !wasPlaying)
			songTxt.text = Language.getPhrase('musicplayer_playing', 'TOCANDO: {1}', [songName]);
		else
			songTxt.text = Language.getPhrase('musicplayer_paused', 'TOCANDO: {1} (PAUSADO)', [songName]);
	...
	public function switchPlayMusic()
	{
		...
		if (playingMusic)
		{
			instance.bottomText.text = Language.getPhrase('musicplayer_tip', 'Aperte ESPAÇO para Pausar/Resumir | ESC para Sair | RESET para Resetar a Música');
			positionSong();
